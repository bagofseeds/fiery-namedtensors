...
4. ~~**`O(1)` compact fast path for `.sel`**~~ — landed, see
   ...
   never materialising `["values"]` or searching it. The endpoints (`k=0`,
   `k=size-1`) are checked directly against the target so an out-of-range
   value clamps/raises exactly regardless of scale; otherwise a
   division-based seed is corrected by walking to the true boundary using
   the real tick value (`origin + k·spacing`, not another division) — exact,
   not epsilon-tolerant, since the naive `round((v − origin) / spacing)`
   loses precision as a *cancellation* error that scales with
   `|origin/spacing|` (an epoch-seconds axis with millisecond spacing, say),
   which a fixed epsilon guard can't absorb at every scale.
